...
BOOST_AUTO_TEST_CASE(d_simple) {
  ...
  BOOST_TEST(testAverageKModel.calculate_d(M_PI / 3, M_PI / 6,
...
BOOST_AUTO_TEST_CASE(simple) {
  BOOST_TEST(testAverageKModel.calculate_psi_d(0, 0, 0) == 0);
}

BOOST_AUTO_TEST_CASE(psi_d_pi_2) {
  BOOST_TEST(testAverageKModel.calculate_psi_d(2000, 1000, 1000) == M_PI / 2,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(same_dote) {
  BOOST_TEST(testAverageKModel.calculate_psi_d(2000, 2000, 0) == 0,
             tt::tolerance(1e-6));
}
...
BOOST_AUTO_TEST_CASE(plot_for_psi_d) {
  ...
  for (int j = 0; j < N; ++j) {
    ...
    y[j] = testAverageKModel.calculate_psi_d(ha, x[j] * 1000, R);
  }
...
BOOST_AUTO_TEST_CASE(simple) {
  BOOST_TEST(testAverageKModel.calculate_psi_g(0, 0, 0) == 0);
}

BOOST_AUTO_TEST_CASE(psi_g_pi_2) {
  BOOST_TEST(testAverageKModel.calculate_psi_g(2000, 1000, 1000) == M_PI / 2,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(same_dote) {
  BOOST_TEST(testAverageKModel.calculate_psi_g(2000, 2000, 0) == 0,
             tt::tolerance(1e-6));
}
