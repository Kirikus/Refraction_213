...
BOOST_AUTO_TEST_CASE(ground_level_psi_d) {
  BOOST_TEST(testModel.R_via_psi_g(0, 0, 0) == 0);
}

BOOST_AUTO_TEST_CASE(ha_2000m) {
  BOOST_TEST(testModel.R_via_psi_d(2000, 0, M_PI / 2) == 2000,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(ha_8000m) {
  BOOST_TEST(testModel.R_via_psi_g(8000, 2000, M_PI / 2) == 6000,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(psi_g_0) {
  BOOST_TEST(testModel.R_via_psi_g(2000, 0, 0) ==
...
BOOST_AUTO_TEST_CASE(d_simple) {
  BOOST_TEST(testModel.d(M_PI / 3, M_PI / 6) == Re * M_PI / 6,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(psi_d)

ModelWithoutRefraction testModel;

BOOST_AUTO_TEST_CASE(simple) { BOOST_TEST(testModel.psi_d(0, 0, 0) == 0); }

BOOST_AUTO_TEST_CASE(equilateral_triangle) {
  BOOST_TEST(testModel.psi_d(2000, 2000, Re + 2000) == M_PI / 6,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(psi_d_pi_2) {
  BOOST_TEST(testModel.psi_d(2000, 1000, 1000) == M_PI / 2,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(same_dote) {
  BOOST_TEST(testModel.psi_d(2000, 2000, 0) == 0, tt::tolerance(1e-6));
}
...
BOOST_AUTO_TEST_CASE(plot_for_psi_d) {
  ...
  for (int j = 0; j < N; ++j) {
    ...
    y[j] = testModel.psi_d(ha, x[j] * 1000, R);
  }
...
BOOST_AUTO_TEST_SUITE(psi_g)

ModelWithoutRefraction testModel;

BOOST_AUTO_TEST_CASE(simple) { BOOST_TEST(testModel.psi_g(0, 0, 0) == 0); }

BOOST_AUTO_TEST_CASE(equilateral_triangle) {
  BOOST_TEST(testModel.psi_g(2000, 2000, Re + 2000) == -M_PI / 6,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(psi_g_pi_2) {
  BOOST_TEST(testModel.psi_g(2000, 1000, 1000) == M_PI / 2,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(same_dote) {
  BOOST_TEST(testModel.psi_g(2000, 2000, 0) == 0, tt::tolerance(1e-6));
}
...
BOOST_AUTO_TEST_CASE(plot_for_psi_g) {
  ...
  for (int j = 0; j < N; ++j) {
    x[j] = (h_min + j * (h_max - h_min) / (N - 1)) / 1000;
    y[j] = testModel.psi_g(ha, x[j] * 1000, R);
  }
...
BOOST_AUTO_TEST_SUITE(hs_via_psi_d)

ModelWithoutRefraction testModel;

BOOST_AUTO_TEST_CASE(ground_level_psi_d) {
  BOOST_TEST(testModel.hs_via_psi_d(0, 0, 0) == 0);
}

BOOST_AUTO_TEST_CASE(equilateral_triangle) {
  BOOST_TEST(testModel.hs_via_psi_d(2000, M_PI / 6, Re + 2000) == 2000,
             tt::tolerance(1e-6));
}

BOOST_AUTO_TEST_CASE(psi_d_90) {
  BOOST_TEST(testModel.hs_via_psi_d(2000, M_PI / 2, 1000) == 1000,
             tt::tolerance(1e-6));
}
...
BOOST_AUTO_TEST_CASE(plot_for_hs_via_psi_d) {
  ...
  for (int i = 0; i < M; ++i) {
    ...
    for (int j = 0; j < N; ++j) {
      ...
      y[j] = testModel.hs_via_psi_d(x[j] * 1000, psi, R) / 1000;
    }
    // pass data points to graphs:
    customPlot.addGraph();
    customPlot.graph(i)->setPen(QPen(Qt::blue));
